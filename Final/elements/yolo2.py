...
class OBJ_DETECTION():
    ...
    def detect(self,main_img):
        ...
        pred = non_max_suppression(pred, conf_thres=0.05, iou_thres=0.45, classes=None)
        items = []
            ...
            for p in pred[0]:
                ...
                try:
                    if 'banana' in self.classes:
                        ...
                        if int(p[5]) == 33:
                            label = self.classes[0]
                        label = self.classes[int(p[5])-49]
                    elif 'sports ball' in self.classes:
                        label = self.classes[int(p[5])-33]
                except:
                    continue
